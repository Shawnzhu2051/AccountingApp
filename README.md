# 小猪记账

一个轻量、极客风的 iOS 记账 App（MVP / iOS 26+ / SwiftUI + SwiftData）。

> 重要说明：本项目为**纯 AI 生产**（从需求澄清、架构设计、编码实现、修 Bug、UI 打磨到导出与报表），由 AI 助手在真实设备/模拟器验收反馈驱动下迭代完成。仓库记录了完整的 AI 开发过程与提交历史。

## 功能概览

### 1) 记账（核心）
最基础但完整的“新增流水”能力。

必填字段（除备注外）：
- 分类（收入/支出分别维护）
- 币种：SGD / RMB / USD
- 收入 / 支出
- 时间（精确到天，默认今天）
- 项目（默认“日常项目”，支持选择其它项目）
- 备注（可为空）

分类体系：
- **支出**
  - 一级：娱乐 / 购物 / 日常 / 出行 / 人情 / 金融 / 医疗 / 住房
  - 二级：
    - 娱乐：聚会、运动、旅游、看剧
    - 购物：数码产品、衣物、酒、书、虚拟产品、其他
    - 日常：吃饭、日用品、水电气网、话费、理发、赌博、其他
    - 出行：地铁、打车、机票、高铁
    - 人情：孝敬家长、红包、礼物
    - 金融：投资、税、罚款、其他
    - 医疗：看病、药物
    - 住房：房租、物管费、其他
- **收入**
  - 工资收入 / 红包收入 / 奖金收入 / 其他收入

### 2) 流水
- 按“月”分组收拢（默认仅展开最近 1 个月，其它月份可点开）
- 支持按项目筛选（默认“日常项目”）
- 交易行 UI 更贴近 iOS 原生风格（支持深色模式）

### 3) 报表
- **按收入/支出分大 Tab**（避免内容过载）
- 支持时间范围筛选
- 支持项目筛选（默认“日常项目”），筛选后全页面数据口径一致
- 分类饼图：展示百分比；点击分类可下钻到分类流水列表
- 多币种展示规则：**分币种展示，不跨币种合计**

### 4) 导出
支持导出指定时间段内的流水：
- **Excel（.xls，Excel 2003 XML Spreadsheet 格式）**：可直接用 Excel / Numbers / WPS 打开
- CSV（兼容 Excel）

导出字段：时间、类型、币种、金额、一级分类、二级分类、项目、备注。

## UI/风格
- 整体风格：轻量、极客、用户友好
- 记账页采用卡片式布局（减少表单“系统设置感”，同时保持对齐与层级清晰）
- 金额颜色遵循更克制的 HIG 风格：金额用主文本色，语义用 badge 表达

## 技术栈
- Swift / SwiftUI
- SwiftData（本地存储）
- Charts（SwiftUI Charts）
- iOS 26+

## 如何运行
1. `git clone` 本仓库
2. Xcode 打开 `AccountingApp.xcodeproj`
3. 选择模拟器或真机目标，点击 Run

## 隐私
- 数据默认仅保存在本地（SwiftData）
- 导出文件生成在临时目录，通过系统分享面板导出

## License
MIT（如需调整可再议）
